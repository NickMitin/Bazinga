...
/**
 * Добавление поля `name` объекту `tag`
 * Добавление поля `url` объекту `tag`
 *
 * Date: 14.08.2014
 * Time: 14:37:56
 */
class bm1408027076_af extends bmIMigrate
{
	...
	public function up()
	{
		...
		$messenge = $this->validateFields([['tag' => 'name'], ['tag' => 'url']]);
		...
		$this->execute("
											INSERT IGNORE INTO
												...
												select p1.dataObjectMapId, p2.dataObjectFieldId from
													(
														select id as dataObjectMapId from `dataObjectMap` where name = 'tag'
								) p1,
		...
		$this->execute("ALTER TABLE
                  `tag`ADD COLUMN `name`  VARCHAR(255) NOT NULL DEFAULT '', ADD COLUMN `url`  VARCHAR(255) NOT NULL DEFAULT '';");
